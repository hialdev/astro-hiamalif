...
<template>
  <div class="m-3 overflow-x-hidden">
    <div class="p-5 md:p-13 mb-5">
      <h5 class="font-handrawn text-xl text-purple-800 dark:text-purple-400">My Cases / Works</h5>
      <h2 class="text-3xl font-medium">Thanks to All My Clients for Trusting Me</h2>
      <div class="max-w-[25em]">Every project is a beautiful journey— the more I learn, the more I realize how much there is to discover.</div>
      ...
      <div class="mt-4">
        <div class="flex items-center gap-1 cursor-pointer">
          <div v-for="(item, index) in categories" 
            ...
            <div class="mb-2 inline-block p-1 px-2 dark:text-stone-800 font-handrawn text-lg bg-slate-50 group-hover:bg-gradient-to-r group-active:bg-gradient-to-r from-indigo-200 via-slate-100 to-purple-200 rounded-full whitespace-nowrap">
              {{ item.title }}
            </div>
            ...
            <div v-if="item.description" 
              ...
              <div v-if="index == 0" class="dark:invert">
              ...
              <div v-if="index > 0 && index <= 2" class="dark:invert">
              ...
              <div v-if="index > 2" class="dark:invert">
                <svg xmlns="http://www.w3.org/2000/svg" width="33" height="35" viewBox="0 0 33 35" fill="none">
              ...
              <div :class="['w-full min-w-[15em] text-slate-900 rounded-2xl text-sm p-4 bg-slate-50 border border-slate-900 dark:bg-stone-900 dark:border-stone-100 dark:text-stone-100',
            ...
            <div class="absolute md:translate-y-[190%] translate-y-[0%] md:group-hover:translate-y-[0%] md:group-active:translate-y-[0%] transition-all flex mt-auto flex-col bg-white/30 backdrop-blur-sm justify-end end-0 bottom-0 start-0 p-6 m-6 rounded-2xl border-2 border-slate-900 shadow-[5px_5px_0px_0px_#000]"> <!-- absolute hover:bg-linear-to-b from-transparent to-slate-900 -->
              <div class="flex items-start gap-2 mt-[-3.8em] md:mt-[-4.5em] ms-[-1em]">
                ...
                <div class="font-handrawn flex items-center gap-1 text-sm md:text-lg">
                  <span v-for="tag in work.tags" :key="tag.id" class="p-1 px-2 bg-gradient-to-r from-indigo-200 via-slate-100 to-purple-200 rounded-full whitespace-nowrap dark:text-stone-800">{{ tag.name }}</span>
                </div>
              </div>
