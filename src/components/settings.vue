...
<template>
  <div ref="dropdownRef" class="fixed bottom-0 start-0 z-[1] h-auto m-5">
      <div v-if="isOpen" class="bg-white/75 dark:bg-black/75 backdrop-blur-md mb-3 bottom-0 start-0 min-w-[13em] rounded-xl">
        <div class="p-2">
          <ul class="flex flex-col gap-1">
            <li class="p-2 px-4 rounded-lg">
              <div class="font-handrawn mb-2">Select Theme</div>
              <SelectTheme :class="`bg-white dark:text-stone-900 rounded-xl`" />
            </li>
            <li class="p-2 px-4 rounded-lg">
      ...
      <div @click="isOpen = !isOpen" :class="`inline-flex backdrop-blur-md items-center cursor-pointer justify-center p-3 bg-white/75 dark:bg-stone-600/30 rounded-xl dark:text-purple-200 hover:text-purple-800 dark:hover:text-purple-200 gap-3 relative ${isOpen ? 'text-purple-800' : ''}`">
